...
export default function Register() {
  ...
  return <main className="min-h-screen grid lg:grid-cols-2" style={{ background: "var(--background)" }}>
    <section className="hidden lg:flex flex-col justify-between p-12" style={{ background: "var(--primary)", color: "white" }}>
      <Link to="/" className="font-display text-2xl font-bold">NutriSiji</Link><div><p className="text-sm uppercase tracking-[0.2em] text-white/70">Platform Praktik Gizi</p><h1 className="mt-4 max-w-md font-display text-4xl font-black leading-tight">Data pangan Indonesia untuk pembelajaran dan layanan gizi yang lebih terstruktur.</h1></div><p className="text-sm text-white/70">Lengkapi profil profesional Anda setelah masuk untuk menyesuaikan perhitungan.</p>
    </section>
    <section className="flex items-center justify-center px-5 py-12"><div className="w-full max-w-md">
      <Link to="/" className="font-display text-xl font-bold lg:hidden" style={{ color: "var(--primary)" }}>NutriSiji</Link><h1 className="mt-8 font-display text-3xl font-black" style={{ color: "var(--foreground)" }}>Buat akun</h1><p className="mt-2 text-sm" style={{ color: "var(--muted-foreground)" }}>Gunakan email institusional atau pribadi Anda. Detail profil dapat dilengkapi dari dasbor.</p>
