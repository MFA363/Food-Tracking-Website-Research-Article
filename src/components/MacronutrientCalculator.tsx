...
export default function MacronutrientCalculator({ tdee }: { tdee: number }) {
  ...
  return <section className="rounded-2xl border p-5 space-y-4" style={{ background: "var(--card)", borderColor: "var(--border)" }} aria-labelledby="macro-title">
    ...
    <p id="macro-hint" className="text-sm" style={{ color: "var(--muted-foreground)" }}>Carbohydrate: 45–65% · Protein: 20–30% · Fat: 10–20%</p>
    <p className="text-xs">The ranges above are project examples, not universal recommendations. WHO suggests total fat at 30% of energy or less for adults to help prevent unhealthy weight gain. <a href="/references" className="underline">Sources and individualization</a></p>
    ...
    <p id="macro-validation" role="status" className="text-sm" style={{ color: error ? "#B45309" : "var(--primary)" }}>Total: {Number.isFinite(total) ? Number(total.toFixed(2)) : "—"}% / 100%. {error}</p>
    ...
    <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>Energy = TDEE × percentage ÷ 100. Carbohydrate and protein: 4 kcal/g; fat: 9 kcal/g. Grams = allocated energy ÷ kcal/g. The ranges above are hints; the total must equal 100%.</p>
    <button onClick={save} disabled={!targets || saving} className="rounded-lg px-4 py-2 text-sm font-semibold disabled:opacity-50" style={{ background: "var(--primary)", color: "white" }}>{saving ? "Saving…" : "Save targets"}</button>
